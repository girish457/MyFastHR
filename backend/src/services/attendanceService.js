...
// Judging what a stored attendance day MEANS is not this file's job any more.
//
// Four screens - the muster grid, the history sheet, the date-wise screen and the day-detail
// drawer - render the same rows, and until 480ef43/955b500 each decided for itself what a row
// meant, so they drifted: OFF on the grid, Absent on the sheet, a drawer whose status letter
// contradicted its own sentence. Those commits routed all four through one set of shared
// helpers; the helpers then still sat in a 4,000-line module that imports `db`, so nothing
// stopped the next edit from putting a query back inside a day-judgment branch.
//
// They now live in ./attendance/dayResolver.js, which requires no database at all. That
// (a) makes a query inside day judgment impossible to reintroduce, (b) is what will later let
// the SAME resolver be called from inside machineAttendanceService's `FOR UPDATE` transaction,
// where an await on a round-trip would let another punch interleave, and (c) makes the
// combinatorial space cheap to test - scripts/testDayResolver.js drives every shift type x
// pinned/unpinned shift x status letter x overlay with plain objects and no MySQL.
//
// What stayed HERE is everything the resolver must not know about: query building, database
// access, data marshalling, and the clock. The clock especially - the resolver takes `now` and
// `todayStr` as arguments, and this file is the only place that reads the real one.
const {
    dbDateToUTC,
    toLocalYMD,
...
// same one the resolver depends on, and it must not change. ATTENDANCE_TROUBLESHOOTING.md
// records `from_date DESC, id DESC` as the fix for the shift assignment priority bug: an
// assignment must resolve by the period it COVERS first and only then by insertion order.
// `id desc` alone picks the most recently CREATED row, which is a different row whenever an
// employee holds overlapping open-ended assignments - 164 of 231 active employees at one
// client. Screens that ordered differently resolved different shifts for the same day:
// measured on one 09:00-18:00 row, the muster said E / QA Evening while the day-detail drawer
// and the history sheet said L / QA Morning.
...
//
// The query half of the pinned-shift rule. The decision half - which of a day's rows the pin
// comes from, and what outranks what - is dayResolver.shiftForDay.
...
// Query + decision. The query (which assignment covers this date, ordered correctly) stays;
// the decision (does that shift cross midnight) is dayResolver.isNightShift.
// NOTE: nothing calls this today - it was already unreferenced before the extraction and is
// carried across unchanged rather than deleted, since removing it is not this change's job.
...
class AttendanceService {
    ...
    async getMatrix(user, month, year) {
        ...
        // dayResolver never reads the clock, so both forms of "now" are sampled here and handed
        // down - the instant for terminate_hour tests, the IST day string for is-it-today tests.
        const now = new Date();
    ...
    async getEmployeeAttendanceHistory(companyId, employeeId, from, to) {
        ...
        // One clock read for the whole screen, handed to the resolver - which never reads it.
        const now = new Date();
        const todayStr = istDateStr(now);
        ...
        return employees.map(emp => {
            ...
            return {
                ...
                status: resolveDayStatus(empLogs, activeShift, rules, {
                    ...
                    weekoffs: empWeekoffs, holidays, leaves: empLeaves, shift: activeShift, todayStr, now
                }).status,
    ...
    async getDayDetail(companyId, employeeId, dateStr) {
        ...
        // The instant this drawer is judged against. The two inline terminate_hour tests below
        // predate the resolver and keep their own `new Date()`; this is the one the resolver gets.
        const now = new Date();
        const todayStr = istDateStr(now);
        ...
        if (attendanceLogs.length > 0) {
            splitShiftDetails = calculateSplitShiftStatus(attendanceLogs, activeShift, rules, now);
        }
